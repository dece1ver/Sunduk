            ...
            if (cornerBlunt > 0)
            {
                switch (bluntType)
                {
                    case Blunt.SimpleChamfer:
                        blunt = $"G1 X{(externalDiameter - 2 * cornerBlunt - tool.CornerRadius - 1).NC(0)} F{feedRough.NC()}\n" +
            ...
            bool doFinish, int speedRough, int speedFinish, double feedRough, double feedFinish)
        {
            ...
            if (profStockAllow > 0)
            {
                ...
                if (outerCornerBlunt > 0)
                {
                    ...
                    $"G1 X{(externalDiameter + bottomStockAllow).NC()} F{feedFinish.NC()}\n" +
            ...
            switch (Math.Abs(tool.Width - width))
            {
                case < 0.001 when outerCornerBlunt > 0:
                    cutting += 
                        $"G0 X{(externalDiameter + clearance).NC()} S{speedFinish}\n" +
                        bluntRight +
                ...
                default:
                {
                    ...
                    {
                        cutting += 
                            $"{(!useCycles ? $"G0 X{(externalDiameter + clearance).NC()} S{speedFinish}\n" : string.Empty)}" +
                            bluntRight +
                            $"X{internalDiameter.NC()}{(innerBluntSize > 0 ? $" {innerBluntLetter}{innerBluntSize.NC()}" : string.Empty)}\n" +
                            $"Z{centerPoint.NC()}\n" +
                            $"G0 X{(externalDiameter + clearance).NC()}\n" +

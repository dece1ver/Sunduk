...
namespace Sunduk.PWA.Infrastructure
{
    ...
    public class Machine
    {
        ...
        /// <summary>
        /// Строка автора/даты в конце шапки программы — построчно, как есть. Плейсхолдеры:
        /// {AUTHOR} (поле "Автор" в наладке — если пусто, схлопывается вместе со своими скобками,
        /// не оставляя "()"), {DATE} (сегодняшняя дата). Placeholder отсутствует в шаблоне — значит
        /// не выводится вовсе, это и определяет, показывать ли автора/дату. Пусто — строка не
        /// добавляется.
        /// </summary>
        ...
        /// <summary>
        /// Шаблон строки вызова инструмента в начале перехода — построчно, как есть. Плейсхолдеры:
        /// {TOOL} (название и параметры инструмента), {T} (номер инструмента без дополнения нулями),
        /// {TN} — номер инструмента с N-значным дополнением нулями (N — любое число, например
        /// {T2}, {T4}), {CS} (система координат перехода, см. <see cref="CoordinateSystems"/> —
        /// пусто, если у станка только одна СК), {COOLANT} (код включения СОЖ перехода — пусто при
        /// типе СОЖ «Без СОЖ»). Перевод строки в шаблоне переносит {CS}/{COOLANT} (или всё, что
        /// после них) на следующую строку УП. Пусто — выводится просто номер инструмента ({T}).
        /// Если в шаблоне нет {CS}/{COOLANT}, СК и включение СОЖ всё равно гарантированно выводятся
        /// отдельными строками сразу после вызова инструмента (когда применимо) — добавление
        /// плейсхолдера в шаблон убирает соответствующую отдельную строку, чтобы не дублировалась.
        /// </summary>
